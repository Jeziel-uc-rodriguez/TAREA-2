...
author: "Jeziel Abisai Uc Rdoriguez"
date: "11/09/2023"
output:
  rmdformats::material:
    highlight: kate
...
1.- ?Cuál es el resultado de `dnormales[order(x)]`, `dnormales[1:length(vec)]`, `dnormales[-20]`, `dnormales[vec>0.5]`?
 ...
 + Obtendremos números aleatoreos
 
2.- ?Qué sucede si hacemos `dnormales[0:3]`? ?Cuál es la longitud del vector?

+ Cuando realizas `dnormales[0:3]` en R, estás intentando seleccionar los elementos de las posiciones 0,1,2 y 3 del vector dnormales. Sin embargo, en R, los indicesde los elementos del vector comienza con 1, no en 0. Por lo tanto estetipo de indexaciónno es valida y genera un resultado incorrecto.

3.- Ahora suponga que `x <- c(1,24,8)` y `uno <- c(T,T,F)`. ?Qué sucede si hacemos `x[uno]`? ?Y si hacemos `x[as.double(uno)]`?

+ x(uno):El resultado será un vector que contiene los elementos de x donde uno es TRUE, es decir los primeros dos elementos de x: c(1,24).

+ x[as.double(uno)]:El resultado será también un nuevo vector que contiene los elementos de x donde as.double(uno) es mayor a 0, es decir, los primeros dos elementos x:c(1,24)
